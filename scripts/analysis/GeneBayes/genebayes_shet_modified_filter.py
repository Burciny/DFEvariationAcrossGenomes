...
def integrate(these_params, these_likelihoods, logit=True):
    ...
    grid = torch.linspace(-1.0, 1.0, N_INTEGRATION_PTS).expand(1, -1)
    means = Prior.get_mean(these_params)
    sds = Prior.get_sd(these_params)
    grid = means.expand(-1, 1) + grid * 8 * sds.expand(-1, 1)

    eval_at_pts, prior_p = log_prob(
        Prior.distribution, these_params, these_likelihoods, grid, logit=logit
    )

    m = torch.max(eval_at_pts, dim=1, keepdim=True)[0]
    pdf = eval_at_pts - m

    result += torch.exp(m.squeeze()) * torch.trapezoid(torch.exp(pdf), grid, dim=1)

    m = torch.max(prior_p, dim=1, keepdim=True)[0]
    check = torch.exp(m.squeeze()) * torch.trapezoid(
        torch.exp(prior_p - m), grid, dim=1
    )
    if np.any(np.abs(check.detach().cpu().numpy() - 1.0) > 1e-5):
        bad_idx = np.argmax(np.abs(check.detach().cpu().numpy() - 1.0))
        print(
            "Discrepancy of ",
            np.max(np.abs(check.detach().cpu().numpy() - 1.0)),
...
class PriorScore(LogScore):
    ...
    def score(self, y, compute_grad=False):
        ...
        if compute_grad:
            ...
            hessian = np.zeros((y.shape[0], self.n_params, self.n_params))
        ...
        for idx in torch.split(
            ...
            if compute_grad:
                this_grad = torch.autograd.grad(
                    result, these_params, retain_graph=True, create_graph=True
                )[0][:, :, 0]
                grad[:, idx_numpy] = [
        ...
        if compute_grad:
            ...
            untransf_grad[Prior.positive] *= self.params_transf[Prior.positive, :]
    ...
    def regularized_hessian(self):
        print(
            "Mean, percentiles of grad norms",
            np.mean(np.sqrt((self.gradient**2).sum(axis=1))),
            np.percentile(
                np.sqrt((self.gradient**2).sum(axis=1)), [0, 25, 50, 75, 100]
            ),
        )
        ...
        print(
            "Mean, percentiles of eigenvalues",
            np.mean(np.abs(evals)),
        ...
        print(
            "After clipping, Mean, percentiles of eigenvalues",
            np.mean(np.abs(clipped_evals)),
            np.percentile(np.abs(clipped_evals), [0, 25, 50, 75, 100]),
        )

        regularized_hessian = (
            np.einsum("nij,nj,nkj->nik", evecs, clipped_evals, evecs)
...
def likelihood(hs, likelihoods):
    ...
    """
    S_GRID = torch.tensor(
        [0.0] + np.exp(np.linspace(np.log(1e-8), 0, num=100)).tolist(), device=DEVICE
    )
    ...
    def distribution(params, logit=True):
        if logit:
            ...
            return dist.TransformedDistribution(
                dist.Normal(params[0], params[1]), transforms=[dist.SigmoidTransform()]
            )
    ...
    def fit(y):
        ...
        for i in range(MAX_EPOCHS_INIT):
            ...
            for idx in torch.split(
                ...
                optimizer.zero_grad()
                these_params = [
                    torch.exp(p[idx]) if Prior.positive[p_idx] else p[idx]
...
def compute_posterior(these_params, y, prob_y):
    ...
    eval_at_pts, prior_p = log_prob(Prior.distribution, these_params, y, grid)
    ...
    grid_points_nonunif = torch.exp(
        torch.linspace(
            torch.log(torch.tensor(5e-8)), torch.log(torch.tensor(0.995)), steps=500
        )
    ).unsqueeze(dim=0)
    post_pdf_nonunif, _ = log_prob(
        Prior.distribution, these_params, y, grid_points_nonunif, logit=False
    )
...
def output_prior_posterior(model, feature_table, y, max_iter=None):
    ...
    params = torch.tensor(
        model.pred_dist(X, max_iter=max_iter)[:].params, device=DEVICE
    )

    # prior
    prior_mean = (
        torch.mean(
            Prior.distribution(params, logit=False).sample(torch.Size([10000])), dim=0
        )
        .detach()
        .cpu()
        .numpy()
    ...
    index = pd.DataFrame({"Gene_name": feature_table.index})
    pdf = pd.DataFrame(
        pdf,
        columns=torch.exp(
            torch.linspace(
    ...
    importance.to_csv(
        args.out_prefix + ".feature_importance.tsv", sep="\t", index=False
    )


def format_lh(GENE_LIKELIHOODS, data):
    lh = []
    for gene in data.index:
        if gene not in GENE_LIKELIHOODS.keys():
            lh.append(torch.zeros([101], device=DEVICE))
        else:
            lh.append(GENE_LIKELIHOODS[gene].to(DEVICE))
    return torch.stack(lh)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1,
        required=False,
        help="Batch size for processing genes during training and integration."
    )
    parser.add_argument(
        "--n_integration_pts",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Experimental regularization parameter for 2nd order optimization. "
        "Unused in the current setup (where natural_gradient=False).",
    )
    parser.add_argument(
        "--hess_reg_2",
        dest="hess_reg_2",
    ...
    feature_table = pd.read_csv(args.features, sep='\t', index_col="Gene_name", low_memory=False)

    genes_in_likelihood = set(y.keys())
    feature_table = feature_table.loc[feature_table.index.intersection(genes_in_likelihood)]

    all_genes = list(set(feature_table.index))
    feature_table = feature_table.loc[all_genes]
    y = format_lh(y, feature_table)

#### !!!! Species specific: This is for mouse
    ### train ###
    train_idx = ~feature_table["chrom"].isin(["chr1", "chr2", "chr3", "chr4"])
    val_idx = feature_table["chrom"].isin(["chr1", "chr2"])
## For yeast
#    train_idx = ~feature_table["chrom"].isin([1,2,3,4])
#    val_idx = feature_table["chrom"].isin([3,4])
# Dmel
#    train_idx = ~feature_table["chrom"].isin(["2", "X"])
#    val_idx = feature_table["chrom"].isin(["2", "X"])


    feature_table = feature_table.drop(columns=["chrom"], errors="ignore")
    X = feature_table.to_numpy()
    ...
    if (
        ...
        if not torch.cuda.is_available():
            print("GPU not available, using CPU (hist) for XGBoost")
            learner = xgb.XGBRegressor(tree_method="hist", **xgb_params)
        else:
            print("GPU available, using GPU (gpu_hist) for XGBoost")
